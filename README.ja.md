...
このカスタムビルド（`5.0.7-pro-context.4`）では、自動 Pro と Zero Risk Pro が、通常はネイティブ Codex の
...
折りたたまれた状態でも **Thinking failed** を検出します。自動実行タスクでは、失敗した応答が
**60 分を超えていた場合のみ**、実行中のツール処理が完了してから同じ会話へ短い続行指示を送ります。
続行の送信が受理されるたびに計時をリセットし、次の失敗が 60 分以下ならエラーで終了します。
...
[長時間応答の続行](docs/adr/0007-thinking-failure-continuation.md)を参照してください。
