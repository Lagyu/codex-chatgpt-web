...
このカスタムビルド（`5.0.7-pro-context.3`）では、自動 Pro と Zero Risk Pro が、ネイティブ Codex の
各ターンにつき新しいユーザーメッセージを 1 件だけ送信します。最初のメッセージにはタスクの初期指示を含め、
以降は新しい入力だけを送信します。すべてのローカルツール／MCP の処理は、そのターンの単一の ChatGPT
アシスタント応答内で続行します。Pro では Codex 履歴の再送、コンテキストの再構築、コンパクション、
自動復旧、分割送信、ステージングを無効にしています。Bigger Context を有効にしても同様です。
...
折りたたまれた状態でも **Thinking failed** を検出したら、明示的なエラーでターンを終了し、自動再送しません。
通常のチャットへの変更は、ChatGPT 側の失敗原因に対する検証済みの修正ではありません。
[通常チャットの方針](docs/adr/0004-regular-chats.md)と[失敗検出](docs/adr/0005-thinking-failed.md)を参照してください。
