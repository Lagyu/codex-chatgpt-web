...
ブリッジは、コンパイル済みの現在の Codex タスクコンテキストを新しい ChatGPT 通常のチャットへ送り、
...
ローカル Responses ブリッジは、選択されたモデルのタスクだけをタスクに紐付いた ChatGPT 通常のチャットへルーティングします。
Full モードでは、MCP が ChatGPT を同じ Codex タスクのツールへ接続します。Pro は Codex の
コンパクションを使わずに ChatGPT の会話を保持し、他の effort は従来のコンテキストライフサイクルを維持します。

## Pro の会話保持ビルド

このカスタムビルド（`5.0.7-pro-context.6`）では、自動 Pro と Zero Risk Pro が、通常はネイティブ Codex の
各ターンにつき新しいユーザーメッセージを 1 件送信します。最初のメッセージにはタスクの初期指示を含め、
以降は新しい入力だけを送信します。すべてのローカルツール／MCP の処理は、そのターンの単一の ChatGPT
アシスタント応答内で続行します。例外は、下記の長時間応答の失敗後の続行だけです。
Pro では Codex 履歴の再送、コンテキストの再構築、コンパクション、再起動後の自動復旧、
分割送信、ステージングを無効にしています。Bigger Context を有効にしても同様です。

インストール後に Launcher と Codex を再起動し、新しい Codex タスクを開始してください。タスクの
ChatGPT タブは開いたままにしてください。会話や進行中の応答が失われた場合は明示的なエラーとなり、
Codex の履歴から復旧できません。ブラウザーの制限を超える新しいメッセージは短くしてください。
既存の Launcher は、保持中のタブを 30 分のアイドル時間後に閉じ、上限の 5 タブに達した場合にも
解放することがあります。いずれの場合も Pro の会話を継続できなくなります。
有効な `model_context_window` と `model_auto_compact_token_limit` の上書き設定は解除してください。
Codex はモデルのメタデータより後にこれらを適用するため、自動コンパクションが再び有効になる場合があります。
設定にかかわらず Pro のコンパクション要求は拒否します。他のモデルのコンテキスト管理は引き続き利用できますが、
大きなステージングメッセージの送信に Pro を代用することはありません。

実装、比較実験、制限については[設計判断の記録](docs/adr/0001-pro-context.md)を参照してください。
...
- **継続的なタスクセッションとネイティブコンパクション。** 連続するメッセージは、タスクに紐付いた
  1 つの通常のチャットを再利用します。Pro 以外では、コンテキスト境界に達すると、保持中のエージェントがチェックポイントを書き、
...
このビルドは常に通常のチャットを開き、アカウントのパーソナライズ設定を変更しません。
チャットには既存の履歴設定が適用され、プロンプトは引き続き OpenAI によって処理されます。
このプロジェクトは非公式です。適用される OpenAI の利用規約とワークスペースポリシーを守る責任は利用者にあります。

折りたたまれた状態でも **Thinking failed** を検出します。自動実行タスクでは、応答時間に関係なく
**ネイティブの 1 ターンにつき最大 5 回**、ツール処理の完了を確認して同じ会話へ続行指示を送ります。
続行メッセージは **Please continue.** と、Codex Native ツール用の新しいトークンを示す 1 行だけです。
待機時間は順に 5、10、20、40、60 秒です。失敗した応答に Stop が残った場合は、その生成だけを停止します。
5 回目の続行も失敗した場合、停止とツール処理の完了を確認したチャットを保持するので、同じ Codex タスクで
**Continue** と送って再開できます。通常の 30 分のアイドル期限と最大 5 タブの制限は引き続き適用されます。
元のプロンプト、履歴、ツール呼び出しは再送しません。手動の Zero Risk、メンテナンス、ステージング、
コンパクション、Luna のチェックポイント取得は自動続行の対象外です。
通常のチャットへの変更は、ChatGPT 側の失敗原因に対する検証済みの修正ではありません。
[通常チャットの方針](docs/adr/0004-regular-chats.md)、[失敗検出](docs/adr/0005-thinking-failed.md)、
[回数制限付きの失敗復旧](docs/adr/0008-bounded-failure-recovery.md)を参照してください。
...
> 最大 270,000 tokens まで拡張されます。ネイティブ Codex compaction は Pro 以外で利用できます。
> このビルドの Pro は分割送信や Codex compaction を使用しません。
...
ブラウザー、通常のチャット、プロンプトコンパイラー、Responses parser、コンパクションハンドラーを通して実行します。
