...
accepted only while the same document, accepted user message and current response are verified.
Cancellation, replaced documents/messages, unsettled tools, uncertain submission, and other errors
stop the task. Manual Zero Risk, smoke tests,
compaction, multipart requests, and Luna checkpoint capture remain outside this policy.

If all five continuations fail, the native turn reports an error but retains the verified,
stopped chat for manual **Continue** in the same Codex task. Its old tool capability is closed;
the new instruction gets a fresh owner and attempt budget. Resume before the normal 30-minute
idle expiry, capacity eviction, app restart, or closing the tab. Unsafe/ambiguous failures are
not advertised as resumable. Installing this update cannot recover tabs already released by `.4`.

The log records `attempt=1/5` through `5/5`, `responseElapsedMs`, `continuation accepted`, and
`browser.tab_failed_retained` for a settled pause. A precise identity-check reason replaces the
old whole-URL comparison error.
The elapsed time ends at detection; it cannot establish an earlier upstream failure timestamp
when the browser was unobservable. See [ADR-0008](docs/adr/0008-bounded-failure-recovery.md).
