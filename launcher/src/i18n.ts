...
const en = {
  ...
  biggerContextBody: "Applies only to non-Pro models. Small turns use one message; large context is split across two or three messages. The final part starts the task. It triples context and compaction thresholds; restart Codex after changing it. Extra requests may increase rate limits or cooldowns. Pro always uses one message and its retained ChatGPT conversation, without Codex compaction or staging. Disabled by default.",
  biggerContextRecommendationTitle: "Use up to 3× more context",
  biggerContextRecommendationBody: "For non-Pro models, Bigger Context sends large tasks in multiple messages to use up to three times more context. Pro is excluded. This experimental feature can increase rate limits or temporary cooldowns.",
  ...
  stepSmokeBody: "Select High, send a small test message, and verify a complete streamed response.",
  ...
  zeroRiskProProfileInfo: "Uses the retained ChatGPT conversation as its only context. Each turn sends one new message; Codex compaction, history replay, recovery, and staging are disabled. Keep the task's chat open and select ChatGPT Pro manually every turn. A Pro account is required. Zero Risk cannot verify your subscription or selection.",
  ...
  manualBiggerContextUnavailable: "Zero Risk prompts are single-part and manual. The default profile uses its existing compaction interval; Pro uses retained ChatGPT context with compaction and staging disabled.",
...
const zh: Record<keyof typeof en, string> = {
  ...
  biggerContextBody: "仅适用于非 Pro 模型。较小回合使用一条消息，较大的上下文拆分为两条或三条消息，最后一部分开始任务。上下文和压缩阈值扩大三倍；更改后请重启 Codex。额外请求可能增加速率限制或临时冷却。Pro 始终只发送一条消息并使用保留的 ChatGPT 对话，禁用 Codex 压缩和分段暂存。默认关闭。",
  biggerContextRecommendationTitle: "使用多达 3 倍的上下文",
  biggerContextRecommendationBody: "对于非 Pro 模型，Bigger Context 通过多条消息发送大型任务，使用多达三倍的上下文。Pro 不使用此功能。此实验功能可能增加速率限制或临时冷却。",
  ...
  zeroRiskProProfileInfo: "仅使用保留的 ChatGPT 对话作为上下文。每回合只发送一条新消息，禁用 Codex 压缩、历史重放、恢复和分段暂存。请保持任务对话打开，并在每个回合手动选择 ChatGPT Pro。需要 Pro 账户；Zero Risk 无法验证你的订阅或选择。",
  install: "安装模型",
  reinstall: "重新安装",
  ...
  manualBiggerContextUnavailable: "零风险提示保持单段并手动发送。默认配置使用现有压缩间隔；Pro 使用保留的 ChatGPT 对话并禁用压缩和分段暂存。",
...
const ja: Record<keyof typeof en, string> = {
  ...
  biggerContextBody: "Pro 以外のモデルに適用されます。小さなターンは 1 件、大きなコンテキストは 2～3 件のメッセージで送り、最後の部分からタスクを開始します。コンテキストと圧縮のしきい値が 3 倍になります。変更後は Codex を再起動してください。追加リクエストでレート制限や一時的なクールダウンが増える場合があります。Pro は常に 1 件の新規メッセージと保持された ChatGPT の会話を使用し、Codex 側の圧縮や分割送信は行いません。初期設定では無効です。",
  biggerContextRecommendationTitle: "最大 3 倍のコンテキストを使用",
  biggerContextRecommendationBody: "Pro 以外のモデルでは、大きなタスクを複数のメッセージで送り、最大 3 倍のコンテキストを使用します。Pro は対象外です。試験的な機能で、レート制限や一時的なクールダウンが増える場合があります。",
  ...
  zeroRiskProProfileInfo: "保持された ChatGPT の会話だけをコンテキストとして使用します。各ターンで送る新規メッセージは 1 件で、Codex 側の圧縮、履歴の再送、復旧、分割送信は無効です。タスクのチャットを開いたままにし、毎回 ChatGPT Pro を手動で選択してください。Pro アカウントが必要です。Zero Risk は契約状況や選択内容を確認できません。",
  install: "モデルをインストール",
  reinstall: "再インストール",
  ...
  manualBiggerContextUnavailable: "ゼロリスクのプロンプトは単一メッセージを手動で送信します。デフォルト設定は既存の圧縮間隔を使用し、Pro は保持された ChatGPT の会話を使用して圧縮と分割送信を無効にします。",
