...
此定制版本（`5.0.7-pro-context.6`）让自动 Pro 和 Zero Risk Pro 通常在每个原生 Codex 轮次中发送一条
...
即使推理面板已折叠，也能检测 **Thinking failed**。自动任务不再要求最短响应时间；工具调用结束后，
**每个原生轮次最多自动继续 5 次**，始终使用经过验证的同一会话。等待时间依次为 5、10、20、40、60 秒。
继续消息仅包含 **Please continue.**，使用 Codex Native 工具时再加一行新的令牌。
若失败响应仍显示 Stop，只会停止该失败的生成。第 5 次继续仍失败时，确认生成停止且工具调用结束后保留聊天，
可在同一 Codex 任务中发送 **Continue** 手动恢复。原有的 30 分钟空闲期限和最多 5 个标签页限制仍然适用。
...
和[有次数上限的失败恢复](docs/adr/0008-bounded-failure-recovery.md)。
